...
def plot_residuals(y_true, y_pred, output_dir):
    ...
    ax.set_title("Residuals vs Predicted (Random Forest)", fontsize=20)
    ax.tick_params(axis="both", labelsize=14)
    fig.tight_layout()
    fig.savefig(
        os.path.join(output_dir, "residuals_scatter_rf.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close(fig)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "residuals_hist_rf.png"), dpi=150)
    plt.close()


def plot_parity_with_marginals(
    y_train, pred_train, y_test, pred_test, r2_train, r2_test, output_dir
):
    ...
    ax_scatter.scatter(
        y_train, pred_train, s=24, alpha=0.7, color="#d95f02", label="Train"
    )
    ax_scatter.scatter(
        y_test, pred_test, s=24, alpha=0.7, color="#1b9e77", label="Test"
    )
    ...
    ax_scatter.plot([min_lim, max_lim], [min_lim, max_lim], "k--", linewidth=1)
    ...
    ax_right.axis("off")
    plt.savefig(os.path.join(output_dir, "parity_joint_rf.png"), dpi=150)
    plt.close()
...
def draw_pdp_panel(ax, curve, gray_color, blue_color, show_legend=False):
...
def plot_pdp_individual(pipeline, X_train, pdp_features, output_dir):
    if not pdp_features:
        print("[PDP] No requested features found; skipping PDP plots.")
        return
    ...
    for curve in curves:
        ...
        fig.tight_layout()
        fig.savefig(
            os.path.join(output_dir, f"pdp_rf_{curve['spec']['file_stem']}.png"),
            dpi=220,
            bbox_inches="tight",
        )
        plt.close(fig)
...
def plot_shap(model, preprocess, X_sample, feature_names, output_dir):
    try:
        ...
        plt.figure()
        shap.summary_plot(
            shap_values, X_trans, feature_names=feature_names, plot_type="bar", show=False
        )
        ax = plt.gca()
        ax.xaxis.set_major_locator(MultipleLocator(5))
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "shap_bar_rf.png"), dpi=150)
        plt.close()
...
def main():
    ...
    seed_results.to_csv(
        os.path.join(OUTPUT_DIR, "metrics_rf_multi_seed.csv"),
        index=False,
        encoding="utf-8-sig",
    )
    print("== Test Metrics Across Random Seeds (RF) ==")
    ...
    plot_pdp_individual(pipeline, X_train, pdp_features, OUTPUT_DIR)
    ...
    metrics_path = os.path.join(OUTPUT_DIR, "metrics_rf.txt")
    with open(metrics_path, "w", encoding="utf-8") as f:
        f.write("== Test Metrics (RF) ==\n")
        f.write(f"R2:   {r2:.6f}\n")
        ...
        f.write(
            "PDP feature files: "
            + ", ".join([f"pdp_rf_{item['file_stem']}.png" for item in pdp_features])
            + "\n"
        )
