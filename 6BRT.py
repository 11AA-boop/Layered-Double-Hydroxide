...
def plot_residuals(y_true, y_pred, output_dir):
    ...
    ax.set_title("Residuals vs Predicted (BTR)", fontsize=20)
    ax.tick_params(axis="both", labelsize=14)
    fig.tight_layout()
    fig.savefig(
        os.path.join(output_dir, "residuals_scatter_bagging.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close(fig)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "residuals_hist_bagging.png"), dpi=150)
    plt.close()
...
def plot_parity_with_marginals(y_train, pred_train, y_test, pred_test, r2_train, r2_test, output_dir):
    ...
    ax_right.axis("off")
    plt.savefig(os.path.join(output_dir, "parity_joint_bagging.png"), dpi=150)
    plt.close()
...
def save_single_pdp_figures(curves, output_dir, gray_color, blue_color):
    for curve in curves:
        ...
        fig.tight_layout()
        fig.savefig(
            os.path.join(output_dir, f"pdp_bagging_{curve['spec']['file_stem']}.png"),
            dpi=220,
            bbox_inches="tight",
        )
        plt.close(fig)
...
def plot_shap(model, preprocess, X_sample, feature_names, output_dir):
    try:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "shap_summary_bagging.png"), dpi=150)
        plt.close()
        plt.figure()
        shap.summary_plot(shap_values.values, X_trans, feature_names=feature_names, plot_type="bar", show=False)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "shap_bar_bagging.png"), dpi=150)
        plt.close()
...
def main():
    ...
    seed_results.to_csv(
        os.path.join(OUTPUT_DIR, "metrics_bagging_multi_seed.csv"),
        index=False,
        encoding="utf-8-sig",
    )
    print("== Test Metrics Across Random Seeds (Bagging) ==")
    ...
    try:
        ...
        feature_names = list(numeric_cols) + list(binary_cols)
    ...
    metrics_path = os.path.join(OUTPUT_DIR, "metrics_bagging.txt")
    with open(metrics_path, "w", encoding="utf-8") as f:
        f.write("== Test Metrics (Bagging) ==\n")
        f.write(f"R2:   {r2:.6f}\n")
