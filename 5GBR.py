...
def plot_residuals(y_true, y_pred, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "residuals_hist_gbr.png"), dpi=150)
    plt.close()
...
def plot_parity_with_marginals(
    ...
    ax_right.axis("off")
    plt.savefig(os.path.join(output_dir, "parity_joint_gbr.png"), dpi=150)
    plt.close()
...
def save_single_pdp_figures(curves, output_dir, gray_color, blue_color):
    for curve in curves:
        ...
        fig.tight_layout()
        fig.savefig(
            os.path.join(output_dir, f"pdp_gbr_{curve['spec']['file_stem']}.png"),
            dpi=220,
            bbox_inches="tight",
        )
        plt.close(fig)
...
def plot_shap(model, preprocess, X_sample, feature_names, output_dir):
    try:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "shap_summary_gbr.png"), dpi=150)
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "shap_bar_gbr.png"), dpi=150)
        plt.close()
